...
@Component({
  ...
  styleUrl: './bill-add-edit.component.scss'
})
export class BillAddEditComponent {
  // Form fields
  Name: string = '';
  quantity: string = '';
  fatContent: Number = 0;
  ...
  calculateRate() {
    // Formula: (Fat % / 100) * kgFatRate [6]
    this.rate = (Number(this.fatContent) / 100) * this.fatRatePerKg;
    this.rate = this.rate.toFixed(2); // Round to 2 decimal places
    this.Amount = (Number(this.quantity) * this.rate).toFixed(2);
  }
  calculateSnf(){
    let snf = (Number(this.clr)/4 ) + (0.2 * Number(this.fatContent)) + 0.36
  }
  ...
  onSubmit(): void {
    ...
    if(this.id) {
      this.commonService.updateEntry(billData).subscribe({
      next: (response) => {
      ...
      this.commonService.createEntry(billData).subscribe({
      next: (response) => {
